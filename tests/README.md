...
python3 tests/test_mail_handoff.py    # 寄信交接：iPhone / Android / 電腦 各走正確路徑
...
| `test_mail_handoff.py` | 用 iPhone / Android / 桌機三種 UA 實際送出表單，驗證：iOS 先喚起 `googlegmail:///co` 再退回 `mailto:`、Android 直接 `mailto:`、桌機開 Gmail 網頁版，三者的收件人／主旨／內文都完整；並確認**手機完全不會連到 `mail.google.com`**（那會弄丟內文）。獨立版與 CyberBiz 版都跑一遍 |
