(function () {
  var _R = window.React, _RD = window.ReactDOM;
  try {
    new MutationObserver(function (recs) {
      recs.forEach(function (r) {
        Array.prototype.forEach.call(r.addedNodes, function (n) {
          var t = (n.tagName === "STYLE" && n.textContent) || "";
          if (t.indexOf("#dc-root") >= 0 && t.indexOf("height:100%") >= 0) n.remove();
        });
      });
    }).observe(document.head, { childList: true });
  } catch (e) {}
  window.__resources = window.__resources || { __embedded: true };
  try {
/* ---- react.js（內嵌，避免執行期打 CDN）---- */
/**
 * @license React
...
(() => {
  ...
  if (!customElements.get('image-slot')) {
    ...
    console.error("[goamazing] 初始化失敗:", e);
  }
  // 這段若被放進 <head>，執行時標記還沒解析出來，getElementById 會拿到 null，
  // .gx-js 掛不上 → 保底樣式永久生效並蓋掉動畫。所以要等 DOM 就緒再試一次。
  function mark() {
    var root = document.getElementById("gx-root");
    if (root) root.classList.add("gx-js");
    return !!root;
  }
  if (!mark()) document.addEventListener("DOMContentLoaded", mark);
  if (_R !== undefined) window.React = _R;
  if (_RD !== undefined) window.ReactDOM = _RD;
})();
